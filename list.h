...
typedef struct Element{
	...
	double val;
	int m2_pos;
	// used for v,i sources with AC specs
	double complex val_complex;
	
	// used for v,i sources with transient specs
	trans_t* trans_data;
...
//insert the sources with transient spec
typedef struct {
	tran_node_t* head;
	tran_node_t* tail;
	int size;
...
void list_init(list_t* list, tran_list_t* tran_list, ac_list_t* ac_list){
	list->head = NULL;
	list->tail = NULL;
	list->size = 0;
...
void list_delete(list_t* list, tran_list_t* tran_list, ac_list_t* ac_list) {
	int i;
	struct Element *current, *next;
	tran_node_t* tran_cur, *tran_next;
	ac_node_t* ac_cur, *ac_next;
...
void ac_list_insert(ac_list_t* ac_list, node_t* node){
	ac_node_t* new_ac_node = malloc(sizeof(ac_node_t));
	new_ac_node->ac_node = node;
	new_ac_node->next = NULL;
	
	if(ac_list->size == 0){
		ac_list->head = ac_list->tail = new_ac_node;
		ac_list->size++;
	}
	else{
		ac_list->tail->next = new_ac_node;
		ac_list->tail = new_ac_node;
		ac_list->size++;
	}
}


void list_insert(list_t* list, tran_list_t* tran_list, ac_list_t* ac_list, char el, char* name, char* node1, char* node2, char* node3, char* node4, char* model_name, double l, double w, double area, double val, tran_spec_t* tran_spec, double mag, double phase){
	...
	if(el == 'd'){
		...
		if( el == 'c' || el == 'l'){
			ac_list_insert(ac_list, new_node);
		}
		
		if( el == 'v' || el == 'l'){
			new_node->m2_pos = gl_m2_counter;
			gl_m2_counter++;
		}
	}
	...
	new_node->val_complex = mag*cos(phase_rad) + I*mag*sin(phase_rad);
...
//returns number of non-zero for array in AC
int get_nz_complex(list_t list){
	int nz = 0;
	node_t* cur = list.head;
	
	while(cur != NULL){
		if( (cur->el == 'r') || (cur->el == 'c') ){
			if( !strcmp(cur->node1,"0") || !strcmp(cur->node2,"0"))
				nz++;
			else
				nz += 4;
